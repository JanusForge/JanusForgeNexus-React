...
import { Terminal, ShieldCheck, RefreshCw, Share2, ExternalLink, Loader2 } from 'lucide-react';
...
export default function NexusPrimeEngine() {
  ...
  // ✅ HANDLER: Strategic Reset (Triggered from Sidebar or Page)
  ...
  // 🔍 RECONSTRUCTION: Load past syntheses from the Archive
  useEffect(() => {
    const loadArchivedSynthesis = async () => {
      ...
      try {
        ...
        console.error("Neural Recovery Failed:", err); 
      }
    };
    loadArchivedSynthesis();
  }, [currentConversationId]);

  // 🌎 SHARING: Generate Public Report Link
  const handleShare = async (id?: string) => {
    const targetId = id || currentConversationId;
    if (!targetId) return;
    try {
      const res = await fetch(`https://janusforgenexus-backend.onrender.com/api/nexus/share/${targetId}`, { method: 'POST' });
      const data = await res.json();
      if (data.shareUrl) {
        ...
        console.error("Sharing Protocol Failure:", err); 
    }
  };

  // 🚀 IGNITION: Stateful & Adversarial Cluster Sync
  const handleIgnition = async (selectedModels: string[] = []) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Ignition System Error:", error); 
    } finally { 
  ...
  return (
    <div className="flex h-screen w-full bg-[#020202]">
      {/* 🏁 SIDEBAR: Neural History & Action Center */}
      <ConversationSidebar 
        onSelectConversation={handleSelectConversation} 
        currentConversationId={currentConversationId}
        onPrint={() => window.print()}
        onDownload={(id) => exportToPDF('synthesis-stage', `Janus-Forge-Report-${id}`)}
        onPublish={(id) => handleShare(id)}
      />
      ...
      <main className="flex-1 flex flex-col relative z-10 bg-black overflow-y-auto custom-scrollbar">
        ...
        {/* 📸 SYNTHESIS STAGE: Targeted for PDF/Print */}
        <div id="synthesis-stage" className="w-full flex flex-col items-center py-10 px-8">
          ...
          {/* STRATEGIC INPUT */}
          <div className="w-full max-w-4xl relative mb-12 no-print">
          ...
          {synthesisResults.length > 0 && (
            <div className="w-full max-w-5xl mb-10 grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in zoom-in duration-700">
              {synthesisResults.map((res, index) => (
                <div key={index} className="ai-card bg-zinc-900/30 border border-white/5 p-8 rounded-[2rem] backdrop-blur-3xl shadow-2xl group hover:border-indigo-500/30 transition-colors">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_8px_#6366f1]" />
                    <h3 className="text-[9pt] font-black uppercase tracking-[0.3em] text-indigo-400 italic">
                      {res.model} Protocol
                    </h3>
                  </div>
          ...
          {/* ACTION BAR (Post-Synthesis) */}
          {synthesisResults.length > 0 && !isSynthesizing && (
            <div className="flex flex-col items-center gap-6 mb-12 no-print">
              <div className="flex items-center gap-4">
                <button onClick={() => handleSelectConversation(null)} className="flex items-center gap-2 px-8 py-3 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-all group">
                ...
                <button onClick={() => handleShare()} className="flex items-center gap-2 px-8 py-3 rounded-full bg-indigo-500/10 border border-indigo-500/30 hover:bg-indigo-500/20 transition-all group">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">
                    {shareUrl ? 'Link Copied' : 'Share Synthesis'}
                  </span>
                </button>
              </div>
              {shareUrl && (
          ...
          {/* MODEL SELECTION UI */}
          <div className="w-full max-w-5xl pb-20 no-print council-builder-ui">
