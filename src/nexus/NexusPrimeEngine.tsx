...
import FlowViewer from '@/components/ui/FlowViewer'; // ✅ IMPORTED GRAPHICS ENGINE
...
export default function NexusPrimeEngine() {
  ...
  // --- (0-4: Effects remain unchanged for stability) ---
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    socket.on('nexus:new_root', (newRoot: any) => { if (!activeThreadId) setChatThread(prev => [newRoot, ...prev]); });
  ...
  useEffect(() => { if (userIsAtBottom) chatEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [chatThread, activeThreadId, userIsAtBottom]);
  ...
  const handleIgnition = async () => {
    ...
    const originalMsg = userMessage; setIsSynthesizing(true); setUserMessage(''); scrollToBottom();
    try {
      const response = await fetch(`${API_BASE_URL}/api/nexus/ignite`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: originalMsg, models: selectedModels, userId: user?.id, conversationId: activeThreadId, parentPostId: activeParentPostId }), });
      const data = await response.json(); if (!activeThreadId && data.conversationId) setActiveThreadId(data.conversationId);
    } catch (e) { setIsSynthesizing(false); console.error("Ignition Error:", e); }
  };
  ...
  return (
    <div className="w-full min-h-screen bg-black text-white flex flex-col items-center overflow-x-hidden font-sans">
      {/* ... (Toasts, Overlays, and Headers remain unchanged) ... */}
      ...
      <main className={`w-full max-w-4xl px-4 flex flex-col items-center pt-32 pb-48 transition-all duration-1000 ${showSuccess ? 'blur-2xl opacity-20' : 'opacity-100'}`}>
        ...
        <div className="space-y-12 w-full">
          ...
          {displayMessages.map((msg) => {
            ...
            // ✅ SURGICAL INJECTION: GRAPHICS ENGINE
            if (match && match[1]) {
              try {
