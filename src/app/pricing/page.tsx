...
// Ensure this matches your production backend URL
const BACKEND_URL = 'https://janusforgenexus-backend.onrender.com';

const NEXUS_PASSES = [
  {
    id: 'pass_24h',
    tier: '24H', // Added for backend synchronization
    name: '24H Nexus Pass',
    description: 'Full command of the Council for one solar cycle. Perfect for focused bursts of synthesis.',
    price: 5,
    hours: 24,
  ...
  {
    id: 'pass_7d',
    tier: '7D', // Added for backend synchronization
    name: '7D Strategic Sprint',
    description: 'Extended authority for deep synthesis sessions and complex pattern mapping.',
    price: 20,
    hours: 168,
  ...
  {
    id: 'pass_30d',
    tier: '30D', // Added for backend synchronization
    name: '30D Eternal Forge',
    description: 'Mastery of the Nexus. Priority processing and permanent archival of your patterns.',
    price: 75,
    hours: 720,
...
function PricingContent() {
  ...
  const handleStripeCheckout = async (pkg: any) => {
    // 1. Force authentication before checkout
    if (!isAuthenticated) {
    ...
    try {
      // 2. The Handshake: Sending the 'tier' to the backend registry
      const response = await fetch(`${BACKEND_URL}/api/stripe/create-session`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tier: pkg.tier, // '24H', '7D', or '30D'
          userId: user?.id,
          hours: pkg.hours
        }),
      });
      ...
      // 3. The Great Leap: Redirecting to Stripe's hosted checkout
      if (data.url) {
      ...
      alert('Neural link failed to synchronize with the payment gateway.');
    } finally {
      ...
      <div className="container mx-auto px-4 max-w-6xl text-center">
        ...
        {isCanceled && (
          <div className="max-w-2xl mx-auto mb-12 bg-amber-500/10 border border-amber-500/50 rounded-2xl p-4 flex items-center gap-4">
            ...
            <p className="text-amber-200 text-[10px] uppercase tracking-widest font-black">
              Handshake Canceled. Your observer status remains unchanged.
            </p>
          </div>
        )}
        ...
        <div className="mb-24">
          ...
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter mb-8 italic uppercase text-white">
            Nexus <span className="text-indigo-600">Access</span>
          </h1>
          <p className="text-gray-500 max-w-xl mx-auto font-medium text-lg italic">
            Observers watch; the Nexus commands. Activate your neural link to engage the Council.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
          {NEXUS_PASSES.map((pass) => (
            <div 
              key={pass.id} 
              className="group relative bg-zinc-950 border border-white/5 rounded-[2.5rem] p-10 hover:border-indigo-500/50 transition-all duration-500 flex flex-col"
            >
              ...
              <p className="text-zinc-500 text-sm mb-12 flex-1 leading-relaxed italic">
                {pass.description}
              </p>
